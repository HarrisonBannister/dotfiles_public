#!/usr/bin/env bash
#
# Symlinks private dotfiles to user's home directory.
...
## Variables ##
# important paths
dotrepo="$HOME/data/projects/dotfiles/" # dotfiles git repo directory
dotdest="$HOME" # usually the home dir
# array of folders in dotfile repo absolute paths only one dir deep
repofolders=($(find "$dotrepo" -maxdepth 1 -type d ! -name "dotfiles" ! -name "." | grep -v .git))
